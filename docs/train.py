...
def aggregate_with_fisher_trace(all_models, all_traces, temperature=0.5):
    """Aggregate model parameters with Fisher-trace weights + temperature scaling."""
    if not all_models:
    ...
    # ========== 改进点：温度缩放的softmax ==========
    log_traces = torch.log1p(traces)          # log(1 + trace)，压缩动态范围
    weights = torch.softmax(log_traces / temperature, dim=0)  # 温度缩放
    ...
    * Load the aggregated state into the model and clear stale optimizer
    ...
    if aggregation_method == 'avg':
        ...
        agg_state = aggregate_with_fisher_trace(
            ...
            temperature=0.5  # 改成你想试的值：0.2, 0.5, 1.0, 2.0
        )
    else:
