"""main.py  – drop-in replacement for krishnanlab/obnbench/main.py.

Changes vs. upstream:
  • Imports ``SGCNModelModule`` from ``obnbench.model``.
  • ``setup_data_module`` / ``main`` route to ``SGCNModelModule`` when
    ``cfg.model.mp_type == 'SGCN'``; all other models use ``ModelModule``
    unchanged.
